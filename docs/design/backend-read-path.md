...
| blob gets per warm doc GET (body-cache hit) | **0** — the §3.4 body cache is LANDED (read-4): cold 1 (the populating fetch) → warm 0 → post-rewrite 1 (new `(blob_key, etag)` ⇒ miss, never stale), pinned in `tests/read_path_counters.rs` |
...
| read-4-bodycache | **LANDED** — `src/store/body_cache.rs` (the §3.4 immutable-key blob-body LRU: `(blob_key, etag)`-keyed from THIS request's authoritative index round, byte-budgeted `SOLID_SERVER_BODY_CACHE_BYTES` (default 256 MiB, `=0` disables) + per-entry cap `SOLID_SERVER_BODY_CACHE_MAX_ENTRY_BYTES` (default budget/8, oversize bypasses), wired into `CompositeStore::read`/`read_at` BELOW the WAC gate). Adversarial tests landed: rewrite ⇒ new-key miss serving the NEW bytes (stale-serve impossible), delete ⇒ 404 (no resurrection), recreate ⇒ fresh fetch, warm-cache anonymous read stays 401 with 0 blob gets (no authz bypass, no speculative byte work), Range over a cached body slices byte-identically. blob gets/op **1→0** on a warm same-version read, pinned in `tests/read_path_counters.rs` + `tests/embedded_read_counters.rs` | read-1 | deterministic (blob gets/op 1→0 warm; 1 cold; 1 post-rewrite) |
