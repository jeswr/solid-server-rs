...
- `bench/ROUND4-PROFILE.md` — a **profiling round**: the active-CPU distribution of the public-doc,
  authed-doc, and listing paths (via `samply`), the honest verdict (the crypto is the floor on the
  authed path; the listing is the only server-compute-bound path), and the one micro-opt it justified.

## Profiling (`bench/profile-run.sh` / `bench/profile-auth.sh`) — where the CPU goes

`samply` LAUNCHES the server (so the macOS `task_for_pid` entitlement isn't needed — a samply-spawned
child is debuggable), an `oha` / the `auth_load` client drives load for the window, then the server is
SIGINT'd (graceful shutdown) so samply writes the profile. Build the server with
`CARGO_PROFILE_RELEASE_DEBUG=1 cargo build --release` first so the binary carries symbols (the
COMMITTED release profile is unchanged). The analysers — `bench/active-split.py` (active-CPU category
split, idle worker-park excluded), `bench/symbolicate.py` (top our-code self frames via `atos` + the
dSYM), `bench/callers.py` (caller of a hot leaf, to tell a park from productive work) — read the
saved `bench/prof-*.json.gz`.

```bash
CARGO_PROFILE_RELEASE_DEBUG=1 cargo build --release
SCENARIO=public  CONC=16 RECORD_SECS=20 ./bench/profile-run.sh
SCENARIO=listing CONC=16 RECORD_SECS=20 CHILDREN=100 ./bench/profile-run.sh
SCEN=doc         CONC=16 RECORD_SECS=22 ./bench/profile-auth.sh   # needs the conformance Keycloak realm up
python3 bench/active-split.py bench/prof-auth-doc.json.gz
```

The saved `bench/prof-*.json.gz` profiles + per-run `bench/prof-*-oha.json` / `bench/prof-server-*.log`
are DEV artifacts — regenerate by re-running; do not commit them.
