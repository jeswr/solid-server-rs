...
//!
//! - the plain `axum::serve` path taps every accepted stream with [`solid_server_rs::nodelay::tap_nodelay`]
//!   (the `ListenerExt` tap), and
//! - the in-process TLS path composes [`solid_server_rs::nodelay::NoDelayAcceptor`] (axum-server's
...
async fn plain_path_tap_sets_nodelay_on_accepted_stream() {
    ...
    let mut tapped = solid_server_rs::nodelay::tap_nodelay(listener);
...
async fn tls_path_nodelay_acceptor_sets_nodelay_on_raw_stream() {
    let acceptor = solid_server_rs::nodelay::NoDelayAcceptor::new();
    let listener = TcpListener::bind("127.0.0.1:0").await.expect("bind");
    let addr = listener.local_addr().expect("local_addr");
